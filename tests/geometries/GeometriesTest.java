...
class GeometriesTest {
    ...
    void testFindIntersections() {
        ...
        Geometries geometries=new Geometries(
                ...
                new Sphere(1, new Point3D(0,1,0)));
        List<Point3D> result=geometries.findIntersections(new Ray(new Point3D(0,4,0),new Vector(0,-4,1)));
        ...
        assertNull(geometries1.findIntersections(new Ray(new Point3D(0,4,0),new Vector(0,-4,1))),"Empty body collection");

        //TC12:Only one shape is cut
        List<Point3D> result2=geometries.findIntersections(new Ray(new Point3D(0,5,0),new Vector(2,-5,0)));
        assertEquals(1,result2.size(),"Wrong number of points");
        assertEquals(List.of(new Point3D(2,0,0)), result2,"Bad intersection");

        //TC13:No shape is cut
        assertNull(geometries.findIntersections(new Ray(new Point3D(0,3,0),new Vector(0,1,1))),"No shape is cut");
    }
}